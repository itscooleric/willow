...
e.node = function decisionTreeNode(options, queueSplit = true){
    ...
    // if (!this.data) this.data = this.tree.data;
    if (queueSplit){
        if (!this.data) {
            this.data = this.tree.data.slice(0);
            debugger;
        }
...
e.tree.prototype = {
    ...
    fit: async function trainModel(options = {}){
        if (options.constructor == Array){
            ...
            let tree = this;
            // Options object
            // Object.keys(options).map(function(a){this[a] = options[a]});

            Object.keys(options).map(a => tree[a] = options[a]);
